...
def apply_displacements(vertices, faces, vert_uvcoords, displacement_map, face_list):
    ''' applying displacements to a mesh
    '''
    print(vert_uvcoords)
    vert_normals = vertex_normals(vertices, faces)
    vert_displacements = F.grid_sample(displacement_map, vert_uvcoords)[:, :, 0]
    displaced_vertices = vertices + vert_normals * vert_displacements
    displaced_vertices[:, face_list] = (vertices + vert_normals * vert_displacements)[:, face_list]
...
# borrowed from https://github.com/daniilidis-group/neural_renderer/blob/master/neural_renderer/vertices_to_faces.py
def face_features(vertice_features, faces):
    """
    :param vertice_features: [batch size, number of vertices, number of features]
    :param faces: [batch size, number of faces, 3]
    :return: [batch size, number of faces, 3, 3]
    """
    assert (vertice_features.ndimension() == 3)
    assert (faces.ndimension() == 3)
    assert (vertice_features.shape[0] == faces.shape[0])
    assert (faces.shape[2] == 3)

    bs, nv = vertice_features.shape[:2]
    bs, nf = faces.shape[:2]
    device = vertice_features.device
    faces = faces + (torch.arange(bs, dtype=torch.int32).to(device) * nv)[:, None, None]
    vertice_features = vertice_features.reshape((bs * nv, -1))
    # pytorch only supports long and byte tensors for indexing
    return vertice_features[faces.long()]
...
def batch_orth_proj(vertices, camera, joint=None, rot_mats=None, flame=None):
    ''' orthgraphic projection
        X:  3d vertices, [bz, n_point, 3]
        camera: scale and translation, [bz, 3], [scale, tx, ty]
    '''
    # Rotation is done around the root joint
    if joint is not None:
        rot_mats = rot_mats[:, :1]
        rot_mats = rot_mats.repeat_interleave(vertices.shape[1], dim=1)

        vertices_rot = vertices - joint[:, None]
        vertices_rot = torch.matmul(rot_mats, vertices_rot[..., None])[..., 0]
        vertices_rot = vertices_rot + joint[:, None]

    else:
        rot_mats = torch.eye(3).expand(vertices.shape[0], vertices.shape[1], -1, -1).to(vertices.device)
        vertices_rot = vertices

    # Find projections for the visible landmarks
    landmarks2d, _ = flame.find_visible_landmarks(vertices_rot, rot_mats)

    camera = camera.clone().view(-1, 1, 3)
    vertices_trans = vertices_rot[:, :, :2] + camera[:, :, 1:]
    vertices_trans = torch.cat([vertices_trans, vertices_rot[:, :, 2:]], 2)
    vertices_proj = camera[:, :, 0:1] * vertices_trans
    vertices_proj[:, :, 1:] = -vertices_proj[:, :, 1:]

    # Process landmarks
    landmarks2d_trans = landmarks2d[:, :, :2] + camera[:, :, 1:]
    landmarks2d_proj = camera[:, :, :1] * landmarks2d_trans
    landmarks2d_proj[:, :, 1:] = -landmarks2d_proj[:, :, 1:]

    return vertices_rot, vertices_proj, landmarks2d_proj


def batch_pers_proj(vertices, camera, joint=None, full_pose=None, flame=None):
    ''' perspective projection
        X:  3d vertices, [bz, n_point, 3]
        camera: scale, translation and focus by tz, [bz, 5], [scale, tx, ty, tz, f/tz]
    '''
    # Rotation is done around the root joint
    if joint is not None:
        rotation = full_pose[:, :4]

        rot_mats = batch_rodrigues(rotation).view(-1, 1, 3, 3)
        rot_mats = rot_mats.repeat_interleave(vertices.shape[1], dim=1)
        ...
        full_pose = torch.zeros(vertices.shape[0], 20, device=vertices.device)
        vertices_rot = vertices

    # Find projections for the visible landmarks
    _, landmarks3d = flame.find_visible_landmarks(vertices_rot, full_pose)

    camera = camera.clone().view(-1, 1, 5)
    vertices_trans = vertices_rot + camera[:, :, 1:4]

    f = camera[:, :, 4] * camera[:, :, 3]

    vertices_proj = camera[:, :, 0:1] * vertices_trans
    vertices_proj = torch.cat([
        vertices_proj[:, :, :2] / vertices_trans[:, :, 2:] * f[:, :, None],
        vertices_proj[:, :, 2:]], dim=2)
    vertices_proj[:, :, 1:] = -vertices_proj[:, :, 1:]

    # Process landmarks
    landmarks3d_trans = landmarks3d + camera[:, :, 1:4]
    landmarks2d_proj = camera[:, :, :1] * landmarks3d_trans / landmarks3d_trans[:, :, 2:] * f[:, :, None]
    landmarks2d_proj[:, :, 1:] = -landmarks2d_proj[:, :, 1:]

    return vertices_proj, landmarks2d_proj[:, :, :2]
